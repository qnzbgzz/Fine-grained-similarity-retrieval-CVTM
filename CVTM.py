...
class LTB(nn.Module):       # LTB（Local Transformer Block）
    """
...
class MedViT(nn.Module):
    def __init__(self, stem_chs, depths, path_dropout, attn_drop=0, drop=0, feature_dim=8,
        ...
        self.feature_head = nn.Sequential(
            nn.Linear(self.stage_out_channels[-1][-1], feature_dim),
            nn.ReLU()  
        )
    ...
    def forward(self, x, return_features=True):
        ...
        x = self.avgpool(x)
        x = torch.flatten(x, 1)
        features = self.feature_head[0](x)  
        if return_features:
            return features
        x = self.feature_head[1](features)
        return x
...
def MedViT_base(pretrained=False, pretrained_cfg=None, **kwargs):
    model = MedViT(stem_chs=[64, 32, 64], depths=[3, 4, 20, 3], path_dropout=0.2, feature_dim=128, **kwargs)
    return model
